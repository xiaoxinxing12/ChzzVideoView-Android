...
public class CHZZMediaController extends FrameLayout {
...
//    private boolean mFullscreenEnabled = false;
    ...
    public CHZZMediaController(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.CHZZMediaController);
        mScalable = a.getBoolean(R.styleable.CHZZMediaController_uvv_scalable, false);
        a.recycle();
        init(context);
    }

    public CHZZMediaController(Context context) {
        super(context);
        init(context);
    }
